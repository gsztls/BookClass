...
public partial class RoomSelect : System.Web.UI.Page
{
    ...
    protected void GridView_BookList_RowDeleting(object sender, GridViewDeleteEventArgs e)  //点击取消预约后执行
    {
        string ID = GridView_BookList.DataKeys[e.RowIndex].Value.ToString(); 
        int JudgeNum = SqlHelper.ExecuteNonQuery(CommandType.Text, "DELETE FROM [BookClass].[dbo].[BookList] WHERE ID ='" + ID + "'");
